...
int sprdwl_intf_init(struct sprdwl_priv *priv, struct sprdwl_intf *intf)
{
	...
	wl_info("%s: ===== INTF_INIT START, hw_type=%d =====\n",
	...
	if (g_intf_ops.max_num < MAX_CHN_NUM) {
		wl_info("%s: register %d ops, hw_type=%d\n",
			__func__, g_intf_ops.max_num, priv->hw_type);

		/* First deinit all channels in case they were registered by BSP driver */
		for (chn = 0; chn < g_intf_ops.max_num; chn++) {
			wl_info("%s: de-init channel %d before re-registering\n",
				__func__, g_intf_ops.hif_ops[chn].channel);
			sprdwcn_bus_chn_deinit(&g_intf_ops.hif_ops[chn]);
		}

		/* Now register all channels */
		for (chn = 0; chn < g_intf_ops.max_num; chn++) {
			wl_info("%s: registering channel %d\n", __func__,
				g_intf_ops.hif_ops[chn].channel);
